...
def fetch_subtitles_ytdlp(video_id: str, language: str = "en") -> Optional[str]:
    """
    Uses yt-dlp to download the best-available subtitles for a YouTube video.
    Returns a plain-text string (joined captions) or None on failure.
    """
    print(f"DEBUG: Attempting yt-dlp fetch for video_id={video_id}")
    # Build the yt-dlp command
    # --skip-download: we only want subtitles
    # --write-auto-sub: fetch autogenerated captions if official ones are missing
    # --sub-lang: language filter
    # --sub-format: best format (usually vtt)
    # -o -: write to stdout (metadata, not subtitles unfortunately)
    # Actually, yt-dlp doesn't stream subtitles to stdout easily without temporary files,
    # but we can hack it or use a temp file.
    
    # Simpler approach: Use --get-url or similar? No.
    # We will use the --print option if supported or just a temp file in /tmp
    temp_prefix = f"/tmp/subs_{video_id}_{int(time.time())}"
    cmd = [
        "yt-dlp",
        "--skip-download",
        "--write-auto-sub",
    ...
    try:
        ...
        # yt-dlp adds the extension depending on format (e.g. .en.vtt)
        # We need to find the file
        expected_extensions = [".en.vtt", ".en.srt", ".en-en.vtt", ".en-en.srt"]
        for ext in expected_extensions:
            file_path = f"{temp_prefix}{ext}"
            if os.path.exists(file_path):
                ...
                # Simple VTT/SRT parser: remove headers and timestamps
                ...
                for line in lines:
                    ...
                    # Remove some more VTT noise if present
                    if line.startswith("<c>"): line = line.replace("<c>", "").replace("</c>", "")
        ...
        # Cleanup any other partial files
        import glob
    ...
    try:
        ...
        try:
            ...
            if not transcript_text:
                print("DEBUG: Falling back to yt-dlp subtitle fetch")
                transcript_text = fetch_subtitles_ytdlp(video_id)
